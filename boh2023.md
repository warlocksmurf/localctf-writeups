...
## Challenge 2
The challenge asked us to find a specific card with its card name, error type and grade. The challenge provided us a CGC cert number to start off.

![image](https://github.com/warlocksmurf/ctf-writeups/assets/121353711/6abd73b5-b584-47e2-91c9-7f10ad99c44c)

Ok we know the card number but it seems that the error type and grade is not present. So I tried to just Google the cert number on Google and found an Instagram reel on a charizard card.

![pokemon](https://github.com/warlocksmurf/ctf-writeups/assets/121353711/6fe90353-0066-478b-8de1-77127eb4a70c)

![spy-kids-lemme-zoom-in-on-that](https://github.com/warlocksmurf/ctf-writeups/assets/121353711/531cb360-f9bb-40ee-a839-1744c6d9b838)

Zooming into the card, the cert seems to be the card we are looking for, and hence the flag too.

## Challenge 3
The challenge asked us to find a specific location shown in 2 videos (1 is a Kpop MV, 1 is the LCK trailer). Watching the video carefully, it seems that the location shown in both videos is a container yard.
...
Inside the zip file, we can see multiple files. So we can attempt the plain-text attack to crack the zip password using the bkcrack tool.

![image](https://github.com/warlocksmurf/ctf-writeups/assets/121353711/97928222-b2b5-4ac1-83a0-bb265d74cda2)

Since the host file will always have ``# Copyright (c) 1993-2009 Microsoft Corp.`` at the start of a file, we can craete our own text file using this phrase to attempt plain-text attack.
...
The decoded text was ran using Powershell and by analyzing the text, we can find a suspcious variable called `$C`. This indicates a C2 server perhaps and by running the command on Powershell the other half of the flag was obtained!  
...
## Challenge 2
This challenge wants us to decipher the flag with SageMath. This challenge was done by my teammate @Nicholas
