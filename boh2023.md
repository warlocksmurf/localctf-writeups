...
Ok we know the card name and number, but it seems that the error type and grade is not present for some reason. So I tried to just Google the cert number for Charizard VSTAR and found an Instagram reel on a charizard card error. Zooming into the card, the cert number proves that this is the card we are looking for, and hence the flag too.
